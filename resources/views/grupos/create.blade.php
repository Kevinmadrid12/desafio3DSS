...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Crear Nuevo Grupo</h1>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('grupos.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('codigo_tutor') is-invalid @enderror" id="codigo_tutor" name="codigo_tutor" required>
                        ...
                        @foreach($tutores as $tutor)
                            <option value="{{ $tutor->codigo }}" {{ old('codigo_tutor') == $tutor->codigo ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    <label for="estudiantes" class="form-label">Estudiantes (Opcional)</label>
                    ...
                    <select class="form-select @error('estudiantes') is-invalid @enderror" id="estudiantes" name="estudiantes[]" multiple size="10">
                        @foreach($estudiantes as $estudiante)
                            <option value="{{ $estudiante->codigo }}" 
                                {{ (is_array(old('estudiantes')) && in_array($estudiante->codigo, old('estudiantes'))) ? 'selected' : '' }}>
